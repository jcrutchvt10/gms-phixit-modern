...
import com.gmsphixit.app.utils.ZipUtils
import com.gmsphixit.app.core.phixit.model.BaseFlagModel
import com.gmsphixit.app.core.phixit.model.ParcelableFlagModel
import com.gmsphixit.app.core.phixit.model.toParcelable
...
class PhixitFlagsService : RootService() {
                    ...
                    try {
                        ...
                        val flags = List(size) {
                            ...
                            when (val type = theory.toInt() and 7) {
                                ...
                                3 -> BaseFlagModel.FloatFlag(
                                    name,
                                    java.lang.Double.doubleToRawLongBits(input.readDouble())
                                )
    ...
    fun encodeAndSave(packageName: String, flags: Map<Int, List<BaseFlagModel>>) {
        try {
                ...
                flagList.forEach { f ->
                    try {
                        ...
                        when (f) {
                            ...
                            is BaseFlagModel.FloatFlag -> cos.writeDoubleNoTag(
                                java.lang.Double.longBitsToDouble(f.value)
                            )
            ...
            try {
                gmsDatabase.execSQL(
                    "UPDATE last_fetch SET serving_version = ? WHERE type = 1",
                    arrayOf(servingVersion)
                )
